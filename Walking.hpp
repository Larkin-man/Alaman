...
void Wait(int minutes=0, int hours=0, int days=0)
{
	timer = 0;
	Minutes += minutes;
	Minutes += (hours * 60);
...
//---------------------------------------------------------------------------
//Шаг в сторону
String Step(const int& xstep, const int& ystep)
{
	//timer = 0;
	place = map->Strings[Y][X]; //точно
	String Mess;
...
String Walk(const int& xstep, const int& ystep)
{
	timer = 0;
	bool first = true;
	place = map->Strings[Y][X]; //точно
	...
	do
	{
		...
		//----------
		Char next = map->Strings[Y+ystep][X+xstep];
		if (Maps.Duration[Pos(next)] == 0)
			break; //Нельзя идти по той местности				*/
	...
	for (int i=0; i<nEventPos; i++)
	{
		if (xy == EventPoss[i])
		{
			...
			for (int j=0;j<nEvents;j++)
			{
				...
				if (i<the)
				{
					String EventTemp = Events->ReadString("events","ev"+IntToStr(j+1)," ");
